...
class MeasurementSet:
    ...
    def visibilities(self, channel_id, time_id, column):
        ...
        for sub_table in table.iter("TIME", sort=True):
            ...
            print('before',data)
            data = np.average(data[:, :, [0, 1]], axis=2)[:, channel_id]
            print('after0',data)
            data_flag = np.any(data_flag[:, :, [0, 1]], axis=2)[:, channel_id]
            ...
            if weight_spectrum is not None:
                # Mimic WSClean
                weight = np.min(weight_spectrum[:, :, [0, 1]], axis=2)[:, channel_id]
                weight[data_flag] = 0
